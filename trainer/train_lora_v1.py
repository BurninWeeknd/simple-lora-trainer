...
def parse_caption_tags(text: str) -> list[str]:
    # "red, bed, ted" -> ["red", "bed", "ted"]
    # keep it simple; you can get fancier later
...
def inject_lora(unet: nn.Module, rank: int, alpha: float, dropout: float) -> int:
    replaced = 0
    modules = dict(unet.named_modules())

    for name, mod in list(modules.items()):
        if isinstance(mod, nn.Linear) and name.endswith(("to_q", "to_k", "to_v", "to_out.0")):
...
def save_lora(unet: nn.Module, path: str, metadata: dict | None = None):
    tensors = {}
...
def build_lora_metadata(cfg: TrainConfig, tag_counter: Counter, trained_words: str) -> dict[str, str]:
    # ComfyUI expects ss_tag_frequency to be a JSON object whose values are dicts of tag->count.
    # So we wrap your counts under a "dataset" key.
    tag_payload = {
        "dataset": dict(tag_counter)
    }

    return {
        # These two drive the UI bits you care about
        "ss_tag_frequency": json.dumps(tag_payload, ensure_ascii=False),
        "ss_trained_words": trained_words,

        # Keep these as strings (safetensors metadata is string->string)
        # Some UIs also use these to classify the file as a kohya-style LoRA.
...
def run_inference_preview_in_memory(
    *,
    ...
    # Text embeds (cond + uncond for CFG)
    text_in = tokenizer(
        [prompt],
        padding="max_length",
        truncation=True,
    ...
    h = w = 512  # v1: keep fixed for now (matches your current SD path)
    latent_h = h // 8
    latent_w = w // 8
    ...
    for i in range(num_images):
        ...
        for t in scheduler.timesteps:
            # Optional scale_model_input if present
            latent_in = latents
...
def train(cfg: TrainConfig):
    ...
    for _, cap_path in dataset:
        ...
        # apply prepend/append exactly like training does
    ...
    # What you want in "Trained Words" box:
    # simplest: most common tags (or all unique)
    ...
    # ---- Cache + bucket compatibility ----
    # Supported: cache latents per bucket resolution.
    ...
    injected = inject_lora(unet, cfg.lora_rank, cfg.lora_alpha, cfg.lora_dropout)
    for m in unet.modules():
        if isinstance(m, LoRALinear):
            m.lora_scale = 1.0

    log(f"STATUS lora_layers={injected}")

    param_groups = [{"params": list(lora_parameters(unet)), "lr": cfg.unet_lr}]

    if train_clip:
        # only include trainable params
        clip_params = [p for p in text_encoder.parameters() if p.requires_grad]
        param_groups.append({"params": clip_params, "lr": cfg.clip_lr})
        ...
        with torch.no_grad():
            for bucket_res, bucket_indices in bucket_map.items():
                ...
                for idx in bucket_indices:
                    ...
                    # Store on CPU to save VRAM; convert to fp16 for compactness/stability on CPU
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for bucket_res, bucket_indices in bucket_map.items():
            ...
            for start in range(0, num_samples, bs):
                ...
                # text encoder: allow grads iff training clip
                if train_clip:
                    if cfg.clip_skip > 0:
        ...
        if cfg.save_every_epochs > 0 and epoch % cfg.save_every_epochs == 0:
            ...
            save_lora(unet, str(out), metadata=metadata)
    ...
    metadata = build_lora_metadata(cfg, tag_counter, trained_words)
    save_lora(unet, str(final_out), metadata=metadata)
